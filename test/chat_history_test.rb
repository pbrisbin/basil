...
class TestChatHistory< Test::Unit::TestCase
  ...
  def test_purge_history
    # two messages logged "here" and one logged elsewhere
    msg_1 = Message.new('to_a', 'from_a', 'from_name_a', 'text 1', 'testing')
    msg_2 = Message.new('to_b', 'from_b', 'from_name_b', 'text 2', 'NOT testing')
    msg_3 = Message.new('to_c', 'from_c', 'from_name_c', 'text 3', 'testing')
    ...
    assert chat_history.empty?,
      "chat history should be purged"

    # pretend we're called in the other chat
    @msg.instance_eval { @chat = 'NOT testing' }

    assert_equal 1, chat_history.length,
      "1 chat should still be in the other chat"
  end
end
